      ...
      <section className="mt-8">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className="rounded-full bg-[color-mix(in_oklab,hsl(var(--accent-lime))_18%,transparent)] px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-[oklch(0.40_0.18_130)] dark:text-[oklch(0.85_0.20_130)]">
              Default · TRE
            </span>
          ...
          {SUGGESTIONS.map((s, i) => (
            ...
            >
              ...
              <p className="mt-3 text-[11px] text-muted-foreground">
                Update inputs in the <Link href="/predict" className="text-primary hover:underline">Predict</Link> form
        ...
        <AnimatePresence mode="wait">
          {aiState === "enriched" && (
            <motion.div
              ...
              className="rounded-2xl border border-primary/40 bg-[color-mix(in_oklab,hsl(var(--primary))_6%,hsl(var(--surface)))] p-6"
            >
              <div className="flex items-center gap-2 text-xs">
                <span className="rounded-full bg-[color-mix(in_oklab,hsl(var(--primary))_15%,transparent)] px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-primary">
                  AI · Gemini
                </span>
          ...
          {aiState === "fallback" && (
            <motion.div
              ...
              transition={{ duration: 0.25 }}
              role="alert"
              className="rounded-2xl border border-[color-mix(in_oklab,hsl(var(--warning))_40%,transparent)] bg-[color-mix(in_oklab,hsl(var(--warning))_8%,hsl(var(--surface)))] p-5"
            >
